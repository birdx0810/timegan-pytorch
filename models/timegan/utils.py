...
from models.timegan.dataset import TimeGANDataset
...
def timegan_trainer(model, data, time, save_path, args):
    ...
    Args:
        ...
        - save_path (str): The path to save model state dict
    ...
    torch.save(args, f"{save_path}/args.pickle")
    torch.save(model.state_dict(), f"{save_path}/model.pt")
    print(f"\nSaved at path: {save_path}")

def timegan_generator(model, T, load_path, args):
    ...
    with open(f"{load_path}/args.pickle", "rb") as fb:
        args = torch.load(fb)
